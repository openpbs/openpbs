...
import time
import shutil
addons_dir = os.getcwd() + os.sep + "cgroups"

print os.getcwd()
print addons_dir

test_job_scripts = ["eatmem.py",
                    "eatmem2.py",
                    "eatmem3.py",
                    "eatmem.c"]
...
class CgroupsTests(PBSTestSuite):
    ...
    def setUp(self):
        ...
        self.hook_init()
        self.job_scripts_init()

    def job_scripts_init(self):
        """ Set up the job scripts to be used for testing """
        print os.getcwd()

        for script in test_job_scripts:
            tmp_file = addons_dir + os.sep + script
            if os.path.isfile(tmp_file):
                shutil.copy(tmp_file, "/tmp/" + script)

    def clean_job_scripts(self):
        """ Set up the job scripts to be used for testing """
        print os.getcwd()

        for script in test_job_scripts:
            tmp_file = "/tmp" + os.sep + script
            if os.path.isfile(tmp_file):
                os.remove(tmp_file)
...
#            self.assertTrue(rv)
    ...
    def test_t2b(self):
        ...
        self.logger.info("Ctime: %s, type: %s" %
                         (self.server.ctime, self.server.ctime))
        rv = self.mom.log_match(".*%s;update_job_usage: " % jid +
                                "Memory usage: vmem=3208\d{2}kb.*",
                                regexp=True,
                                max_attempts=3,
                                starttime=now)
        self.assertTrue(rv)
    ...
    def test_t4(self):
        ...
        a = {'job_state': 'R'}
        self.server.expect(JOB, a, jid)
        rv = self.mom.log_match(".*vntype: no_cgroups is in " +
                                "\['no_cgroups'\].*",
                                regexp=True,
                                max_attempts=5,
                                starttime=self.server.ctime)
    ...
    def test_t4c(self):
        ...
        a = {'job_state': 'R'}
        self.server.expect(JOB, a, jid)
        rv = self.mom.log_match(".*cgroup excluded for subsystem " +
                                "%s on host %s.*" %
...
if [ -d /cgroup/cpuset/pbspro/$PBS_JOBID ]; then
    base_dir=/cgroup/cpuset/pbspro
    ls -1 $base_dir
    cpus=`cat $base_dir/$PBS_JOBID/cpuset.cpus`
    echo "CpuSocket=${cpus}"
    mems=`cat $base_dir/$PBS_JOBID/cpuset.mems`
    echo "MemorySocket=${mems}"
    base_dir=/cgroup/memory/pbspro
    if [ -d $base_dir/$PBS_JOBID ]; then
       mem_limit=`cat $base_dir/$PBS_JOBID/memory.limit_in_bytes`
       memsw_limit=`cat $base_dir/$PBS_JOBID/memory.memsw.limit_in_bytes`
       echo "MemoryLimit=${mem_limit}"
       echo "MemswLimit=${memsw_limit}"
    fi
elif [ -d /sys/fs/cgroup/cpuset/pbspro/$PBS_JOBID ]; then
    base_dir=/sys/fs/cgroup/cpuset/pbspro
    ls -1 $base_dir
    cpus=`cat $base_dir/$PBS_JOBID/cpuset.cpus`
    echo "CpuSocket=${cpus}"
    mems=`cat $base_dir/$PBS_JOBID/cpuset.mems`
    echo "MemorySocket=${mems}"
    base_dir=/sys/fs/cgroup/memory/pbspro
    if [ -d $base_dir/$PBS_JOBID ]; then
       mem_limit=`cat $base_dir/$PBS_JOBID/memory.limit_in_bytes`
       memsw_limit=`cat $base_dir/$PBS_JOBID/memory.memsw.limit_in_bytes`
       echo "MemoryLimit=${mem_limit}"
