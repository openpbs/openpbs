...
"""
        ...
        try:
            ...
            pass
        rv = self.server.log_match("Job;%s;server is %s" % ("newjob",self.server.shortname), max_attempts=3)
        self.assertTrue(rv)

        rv = self.mom.log_match("Job;%s;server is %s" % (jid,self.server.shortname), max_attempts=3)
        self.assertTrue(rv)
