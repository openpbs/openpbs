...
"""
    ...
    def test_epilogue_single_node(self):
        """
        Test accumulatinon of resources of a single node job from an
        exechost_epilogue hook. The string values specified are used
        by mom as is; that is, they're not converted into JSON format.
        """
...
"""
        ...
        # For string_array type  resource 'stra', it is not accumulated but
        # will be set to last seen value from a mom epilogue hook.
        foo_str3_val = '{"a":6,"b":"some value #$%^&*@","c":54.4,"d":"32.5gb"}'
        self.server.expect(JOB, {
            'job_state': 'F',
            ...
            'resources_used.foo_str3': foo_str3_val,
            'resources_used.stra': "\"glad,elated\",\"happy\"",
        ...
        acctlog_match = 'resources_used.foo_str3='
        acctlog_match += '\{"a"\:6\,"b"\:"some value \#\$\%\^\&\*\@"\,'
        acctlog_match += '"c"\:54\.4\,"d"\:"32\.5gb"\}'
        s = self.server.accounting_match(
            "E;%s;.*%s.*" % (jid, acctlog_match), regexp=True, n=100)
        self.assertTrue(s)
        acctlog_match = 'resources_used.stra=\"glad\,elated\"\,\"happy\"'
