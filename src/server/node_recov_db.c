 ...
 */
struct pbsnode * node_recov_db(void *nd) {
	...
	attr_info.parent_id = dbnode->nd_name;
	attr_info.parent_obj_type = PARENT_TYPE_NODE; /* node attr */

	/* read in que attributes */
	if (recov_attr_db(conn, dbnode, &attr_info, node_attr_def, np->nd_attr,
		(int)ND_ATR_LAST, 0) != 0)
		goto db_err;
...
node_recov_db_raw(void *nd, pbs_list_head *phead)
{
	...
	attr_info.parent_id = (char *) nd;
	attr_info.parent_obj_type = PARENT_TYPE_NODE; /* node attr */

	/* read in que attributes */
	if (recov_attr_db_raw(conn, &attr_info, phead) != 0)
		goto db_err;
 ...
 */
int
node_save_db(struct pbsnode *pnode, int mode)
{
	...
	/*
	 * Delete the node and write it afresh
	 * Deleting the node removes all attributes automatically
	 * Thus when reinserting, we add only the set attributes
	 */
	pbs_db_delete_obj(conn, &obj);

	if (pbs_db_insert_obj(conn, &obj) != 0) /* has to insert */
		goto db_err;

	/* node_attrs */
	attr_info.parent_obj_type = PARENT_TYPE_NODE; /* node attr */
	attr_info.parent_id = dbnode.nd_name;
	attr_info.attr_flags = 0;

	attr_obj.pbs_db_obj_type = PBS_DB_ATTR;
	attr_obj.pbs_db_un.pbs_db_attr = &attr_info;
	...
	for (j = 0; j < ND_ATR_LAST; ++j) {
		...
		(void) node_attr_def[j].at_encode(&pnode->nd_attr[j],
			&wrtattr, node_attr_def[j].at_name,
			NULL, ATR_ENCODE_SVR, NULL);
	}

	/* now that they are encoded, write them out */
	wrote_np = 0;
	while ((psvrl = (svrattrl *) GET_NEXT(wrtattr)) != NULL) {

		/* need to special case writting of certain attributes */
		...
		strcpy(attr_info.attr_name, psvrl->al_name);
		if (psvrl->al_resc)
			attr_info.attr_resc = psvrl->al_resc;
		else
			attr_info.attr_resc = "";
		attr_info.attr_value = psvrl->al_value;
	...
	if (wrote_np == 0) {
		...
		strcpy(attr_info.attr_name, ATTR_NODE_pcpus);
		attr_info.attr_resc = "";
		sprintf(pcpu_str, "%ld", pnode->nd_ncpus);
		attr_info.attr_value = pcpu_str;
	...
	if (((pnode->nd_attr[ND_ATR_Sharing].at_flags & (ATR_VFLAG_SET | ATR_VFLAG_DEFLT)) == (ATR_VFLAG_SET | ATR_VFLAG_DEFLT)) &&
		...
		strcpy(attr_info.attr_name, ATTR_NODE_Sharing);
		attr_info.attr_resc = "";
		attr_info.attr_value = vn_str;
