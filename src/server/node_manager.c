	...
	enum Set_All_State_When setwhen)
{
	...
	for (nchild = 0; nchild < psvrmom->msr_numvnds; ++nchild) {
		...
		pvnd->nd_attr[(int)ND_ATR_state].at_flags |= ATR_VFLAG_MODIFY | ATR_VFLAG_MODCACHE;
			...
			for (psn = np->nd_psn; psn; psn = psn->next) {
				for (pjinfo = psn->jobs; pjinfo; pjinfo = pjinfo_nxt) {
					...
					if (nname && (strcasecmp(np->nd_name, nname) == 0)) {
						...
						pj->ji_wattr[(int)JOB_ATR_exit_status].at_flags |= (ATR_VFLAG_SET | ATR_VFLAG_MODIFY | ATR_VFLAG_MODCACHE);
...
unset_resv_retry(resc_resv *presv)
{
	...
	presv->ri_wattr[(int)RESV_ATR_retry].at_flags |= ATR_VFLAG_MODIFY | ATR_VFLAG_MODCACHE;
...
set_resv_retry(resc_resv *presv, long retry_time)
{
	...
	presv->ri_wattr[(int)RESV_ATR_retry].at_flags |= ATR_VFLAG_SET | ATR_VFLAG_MODIFY | ATR_VFLAG_MODCACHE;
				...
				if (from_hook ||
					...
					if (psrp->vna_val[0] == '@') {
						...
						if (from_hook) {
							...
							prs->rs_value.at_flags |= ATR_VFLAG_MODIFY | ATR_VFLAG_MODCACHE;
							if (psrp->vna_val[0] != '\0') {
	...
	for (i = 0; i < svr_totnodes; i++) {
		if (pnode != pbsndlist[i]) {
			...
			if (host_str1 && host_str2 &&
				...
				(*pala).at_flags = ATR_VFLAG_SET | ATR_VFLAG_MODIFY | ATR_VFLAG_MODCACHE | ATR_VFLAG_DEFLT;
	...
	switch (command) {
		...
		case IS_UPDATE2:
			...
			if (psvrmom->msr_numvnds > 0) {
				...
				np->nd_attr[(int)ND_ATR_pcpus].at_flags |= ATR_VFLAG_SET | ATR_VFLAG_MODIFY | ATR_VFLAG_MODCACHE;
			}
			...
			for (ivnd = 0; ivnd < psvrmom->msr_numvnds; ++ivnd) {
				...
				if (!(np->nd_attr[(int)ND_ATR_ResvEnable].at_flags & ATR_VFLAG_SET) ||
					...
					if (change || !(np->nd_attr[(int)ND_ATR_ResvEnable].at_flags & ATR_VFLAG_SET) || !(np->nd_attr[(int)ND_ATR_ResvEnable].at_flags & ATR_VFLAG_DEFLT))
						np->nd_attr[(int)ND_ATR_ResvEnable].at_flags |= ATR_VFLAG_SET | ATR_VFLAG_DEFLT | ATR_VFLAG_MODIFY | ATR_VFLAG_MODCACHE;
				}
...
update_FLic_attr(void)
{
	...
	pbs_float_lic->at_flags |= ATR_VFLAG_MODIFY | ATR_VFLAG_MODCACHE;
}
...
update_job_node_rassn(job *pjob, attribute *pexech, enum batch_op op)
{
	...
	if (sysru) {
		...
		if (pr) {
			...
			pr->rs_value.at_flags |= ATR_VFLAG_SET | ATR_VFLAG_DEFLT | ATR_VFLAG_MODIFY | ATR_VFLAG_MODCACHE;
		}
	}
	if (queru) {
		pr = find_resc_entry(queru, prdef);
