	...
	if ((pjob->ji_qs.ji_state == JOB_STATE_MOVED) ||
		...
		*newstate = JOB_STATE_QUEUED;
		...
		if (pjob->ji_qs.ji_svrflags&JOB_SVFLG_ArrayJob) {
			...
			struct ajtrkhd  *ptbl = pjob->ji_ajtrk;
			if (ptbl) {
				if (ptbl->tkm_subjsct[JOB_STATE_QUEUED] >= ptbl->tkm_ct) {
...
allocate_cpu_licenses(job *pjob)
{
	...
	/* see if we need to get additional licenses */
	if ( ext_license_server && \
		(cpulic_need > licenses.lb_aval_floating) &&
	( (cpulic_need-licenses.lb_aval_floating) <= \
						licenses.lb_glob_floating) ) {

		cpulic_add = cpulic_need - licenses.lb_aval_floating;

		cpulic_got = pbs_get_licenses(cpulic_add);
		if (cpulic_got > 0) {
			licenses.lb_aval_floating += cpulic_got;
			licenses.lb_glob_floating -= cpulic_got;
		}
		sprintf(log_buffer,
			"Asked external license server for %d cpu licenses, got %d",
			cpulic_add, cpulic_got);
		DBPRT(("%s\n", log_buffer))
		log_event(PBSEVENT_DEBUG3, PBS_EVENTCLASS_JOB,
			LOG_DEBUG, pjob->ji_qs.ji_jobid, log_buffer);

		if (cpulic_got == 0)
			return;	/* no progress - no work to do */
	}
...
relicense_svr_unlicensedjobs(void)
{
	...
	for (pjob = (job *)GET_NEXT(svr_unlicensedjobs);
		...
		/* first see if we need to checkout more cpu licenses    */
		/* from external license server, if we don't have enough */
		/* licenses from the local pool. 			 */
		if( ext_license_server && \
			(job_needs_min >  licenses.lb_aval_floating) && \
			( (job_needs_min-licenses.lb_aval_floating) <=  \
						licenses.lb_glob_floating) ) {
			int cpulic_add;
			int cpulic_got;

			cpulic_add = job_needs_min - licenses.lb_aval_floating;
