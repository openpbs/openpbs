 ...
 *	CrayXMLstart()			-	helper function for XML parsing of ATTR_NODE_TopologyInfo
 *	hwlocXMLstart()			-	helper function for XML parsing of ATTR_NODE_TopologyInfo
 *	XMLend()				-	helper function for XML parsing of ATTR_NODE_TopologyInfo
 *	Nsockets_from_topology()-	count the number of sockets in an XML topology string.
...
/**
 * @brief
 * 		helper function for XML parsing of ATTR_NODE_TopologyInfo
 *
 * @par MT-Safe:	yes
 * @par Side Effects:
 *		sets "nsockets"
 *
 * @par Note:	see documentation for XML_SetStartElementHandler() and
 *		XML_SetEndElementHandler() in the expat documentation
 *		(doc/reference.html, in downloads available at the
 *		SourceForge project page, http://sourceforge.net/projects/expat/)
 *		and the Cray BASIL specification (URL below) for the format
 *		of the BASIL inventory response.
 *
 *		This version of PBS does not yet support the BASIL 1.3 inventory
 *		that directly reports socket elements (nor, as of this writing,
 *		does Cray). Instead, we infer the socket count based on the
 *		number of "Node" elements.
 *
 * @see:	http://spectrum.prog.altair.com:9000/attachments/2011x01/basil12_20101212.pdf
 * @see:	http://spectrum.prog.altair.com:9000/attachments/2012x03/basil13.pdf
 *
 * @param[in]	data	-	not used here
 * @param[in]	elem	-	compares with value "Node"
 * @param[in]	attr	-	not used here
 *
 * @return	void
 */
static void XMLCALL
CrayXMLstart(void *data, const char *elem, const char **attr)
{
	if (!strcmp(elem, "Node"))
		nsockets += 2;	/* two sockets per node */
}

/** @fn hwlocXMLstart
 * @brief	helper function for XML parsing of ATTR_NODE_TopologyInfo
 *
 * @par MT-Safe:	yes
 * @par Side Effects:
 *	sets "nsockets"
 ...
 *
 * @param[in]	data	-	not used here
 * @param[in]	elem	-	compares with value "object"
 * @param[in]	attr	-	attribute
 *
 * @return	void
 */
static void XMLCALL
hwlocXMLstart(void *data, const char *elem, const char **attr)
{
	int	i;
 ...
 *		None
 *
 * @par Note:	see documentation for XML_SetEndElementHandler() and
 *		XML_SetEndElementHandler() in the expat documentation
 *		(doc/reference.html, in downloads available at the
 *		SourceForge project page, http://sourceforge.net/projects/expat/)
 */
static void XMLCALL
XMLend(void *data, const char *elem)
{
}

/**
 * @brief	count the number of sockets in an XML topology string.
 *		If the topology string is a BASIL inventory from a Cray,
 *		we count the number of "Node" objects;  if from hwloc,
 *		we count the number of sockets.
 *
...
{
	static XML_Parser	p = NULL;	/* created only once */

	nsockets = 0;
	if ((p == NULL) && (p = XML_ParserCreate(NULL)) == NULL) {
		log_event(PBSEVENT_DEBUG3, PBS_EVENTCLASS_SERVER, LOG_DEBUG,
			__func__, "failed to create XML parser");
		return (0);
	}
	switch ((int) type) {
		case (int) tt_hwloc:
			XML_SetElementHandler(p, hwlocXMLstart, XMLend);
			break;
		case (int) tt_Cray:
			XML_SetElementHandler(p, CrayXMLstart, XMLend);
			break;
		default:
			log_err(PBSE_INTERNAL, __func__, "unexpected topology type");
			return (0);
	}
	if (XML_Parse(p, topology_str, strlen(topology_str), 1) ==
		XML_STATUS_ERROR) {
		sprintf(log_buffer, "parse error:  %s",
			XML_ErrorString(XML_GetErrorCode(p)));
		log_event(PBSEVENT_DEBUG3, PBS_EVENTCLASS_SERVER, LOG_DEBUG,
				__func__, log_buffer);
	}
	if (XML_ParserReset(p, "UTF-8") != XML_TRUE) {
		sprintf(log_buffer, "XML_ParserReset error %s",
			XML_ErrorString(XML_GetErrorCode(p)));
		log_event(PBSEVENT_DEBUG3, PBS_EVENTCLASS_SERVER, LOG_DEBUG,
				__func__, log_buffer);
	}
...
#else
static int
Nsockets_from_topology(char *topology_str)
{
	/*
	 *	On Windows, we currently don't support socket-based licensing.
	 */
	return (0);
}
#endif	/* WIN32 */
...
set_node_topology(attribute *new, void *pobj, int op)
{
	...
	switch (op) {
		...
		case ATR_ACTION_ALTER:
			...
			if (ppnli->at_flags & ATR_VFLAG_SET) {
				...
				if (ppnli->at_val.at_long < node_nsockets) {
					 ...
					 *	is now reporting more sockets than have
					 *	previously been licensed.  Since it's an
					 *	all-or-nothing license for the node, we
					 *	can't simply ignore the fact that more
					 *	sockets were added:  we reset the node's
					 *	License and LicenseInfo attributes.
					 */
					...
					sprintf(log_buffer, msg_sockets_initialized,
						...
						node_nsockets == 1 ? "" : "s");
					log_event(PBSEVENT_DEBUG3,
						PBS_EVENTCLASS_SERVER, LOG_DEBUG,
						__func__, log_buffer);
					...
					ppnl->at_flags  |= ATR_VFLAG_SET |
						ATR_VFLAG_MODIFY | ATR_VFLAG_MODCACHE;
					pnode->nd_modified |= NODE_UPDATE_OTHERS;
					sprintf(log_buffer, msg_node_licensed,
						ND_LIC_TYPE_locked,
						pnode->nd_name);
					log_event(PBSEVENT_DEBUG3, PBS_EVENTCLASS_SERVER,
						LOG_DEBUG, __func__, log_buffer);
				}
			} else if ((node_nsockets > 0) &&
				licstate_is_up(LIC_SOCKETS) &&
