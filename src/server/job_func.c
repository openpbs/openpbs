...
#endif
	return ((job *)find_tree(AVL_jobs, (void *)buf));
}
...
resv_purge(resc_resv *presv)
{
	...
	if (resv_tree_op(presv, TREE_OP_DEL) != 0) {
		log_event(PBSEVENT_RESV, PBS_EVENTCLASS_RESV, LOG_WARNING, presv->ri_qs.ri_resvID,
				"Failed to unlink resv from avl-lookup tree");
	}
