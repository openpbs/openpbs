...
set_node_ct(resource *pnodesp, attribute *pattr, void *pobj, int type, int actmode)
{
	...
	pnct->rs_value.at_flags |= ATR_VFLAG_SET | ATR_VFLAG_MODIFY | ATR_VFLAG_MODCACHE;
	...
	if (((pncpus->rs_value.at_flags & (ATR_VFLAG_SET | ATR_VFLAG_DEFLT)) ==
		...
		pncpus->rs_value.at_flags |= (ATR_VFLAG_SET | ATR_VFLAG_MODIFY | ATR_VFLAG_MODCACHE);
	}
...
to_kbsize(char *val)
{
	...
	switch (*pc) {
		...
		default:	return (0);	 /* invalid string */
	}
	if (*pc != '\0')
		pc++;
	if (*pc != '\0') {
 ...
 *
 */
int zero_or_positive_action(resource *presc, attribute *pattr, void *pobj, int type, int actmode)
{
	long l;
	if ((actmode != ATR_ACTION_ALTER) && (actmode != ATR_ACTION_NEW))
        	return 0;
