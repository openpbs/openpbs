...
action_sched_iteration(attribute *pattr, void *pobj, int actmode)
{
	if (pobj == dflt_scheduler) {
			...
			server.sv_attr[SRV_ATR_scheduler_iteration].at_flags |= VALUE_SET;
			svr_save_db(&server);
	}
	return PBSE_NONE;
}
...
set_sched_default(pbs_sched *psched, int from_scheduler)
{
	...
	if ((psched->sch_attr[(int)SCHED_ATR_log_events].at_flags & ATR_VFLAG_SET) == 0) {
		...
		psched->sch_attr[SCHED_ATR_log_events].at_flags |= VALUE_SET | ATR_VFLAG_DEFLT;
	}
	...
	if (!(psched->sch_attr[SCHED_ATR_preempt_queue_prio].at_flags & ATR_VFLAG_SET)) {
		...
		psched->sch_attr[SCHED_ATR_preempt_queue_prio].at_flags |= VALUE_SET | ATR_VFLAG_DEFLT;
	...
	if (!(psched->sch_attr[SCHED_ATR_preempt_prio].at_flags & ATR_VFLAG_SET)) {
		...
		psched->sch_attr[SCHED_ATR_preempt_prio].at_flags |= VALUE_SET | ATR_VFLAG_DEFLT;
	...
	if (!(psched->sch_attr[SCHED_ATR_preempt_order].at_flags & ATR_VFLAG_SET)) {
		...
		psched->sch_attr[SCHED_ATR_preempt_order].at_flags |= VALUE_SET | ATR_VFLAG_DEFLT;
	...
	if ( !from_scheduler && !(psched->sch_attr[SCHED_ATR_preempt_sort].at_flags & ATR_VFLAG_SET)) {
		...
		psched->sch_attr[SCHED_ATR_preempt_sort].at_flags |= VALUE_SET | ATR_VFLAG_DEFLT;
	...
	if (!(psched->sch_attr[SCHED_ATR_server_dyn_res_alarm].at_flags & ATR_VFLAG_SET)) {
		...
		psched->sch_attr[SCHED_ATR_server_dyn_res_alarm].at_flags |= VALUE_SET | ATR_VFLAG_DEFLT;
	}
	if (!(psched->sch_attr[SCHED_ATR_job_run_wait].at_flags & ATR_VFLAG_SET)) {
		...
		psched->sch_attr[SCHED_ATR_job_run_wait].at_flags |= VALUE_SET | ATR_VFLAG_DEFLT;
	}
	if (!(psched->sch_attr[SCHED_ATR_throughput_mode].at_flags & ATR_VFLAG_SET) &&
		...
		psched->sch_attr[SCHED_ATR_throughput_mode].at_flags |= VALUE_SET | ATR_VFLAG_DEFLT;
	}
