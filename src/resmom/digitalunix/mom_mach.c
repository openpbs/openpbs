...
mom_get_sample()
{
	...
	nproc = 0;
	errno = 0;
	for (fd = -1; (dent = readdir(pdir)) != NULL; close(fd), errno = 0) {
	...
	if (errno != 0 && errno != ENOENT) {
		sprintf(log_buffer, "readdir error");
		log_err(errno, __func__, log_buffer);
	}
...
mem_proc(pid_t pid)
{
	...
	sprintf(ret_string, "%lukb", (pi->pr_size*page_size) >> 10); /* in KB */
	return ret_string;
}
...
resi_job(pid_t jobid)
{
	...
	if (found) {
		/* in KB */
		sprintf(ret_string, "%lukb", (resisize * page_size) >> 10);
		return ret_string;
	}
...
/**
 * @brief
 *	no special function required to set priority of processes
 *
 */
void
mom_nice()
...
/**
 * @brief
 *      no special function required to unset priority of processes
 *
 */
void
mom_unnice()
