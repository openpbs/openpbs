 ...
 *
 *	Parts of this code were developed using information acquired
 *	by consulting code from the Condor Design Team.
 */
...
/**
 * @brief
 *	Do mach stuff for apple since they didn't bother to
 *	completely port the kvm stuff.
 */
int
apple_info(int index)
{
	struct	kinfo_proc	*kp = &proc_tbl[index];
	struct	pstats		*ps = &pstat_tbl[index];
	task_t			subject;
	kern_return_t		status;
	task_basic_info_data_t	TaskInfo = { 0 };
	...
	status = task_for_pid(mach_task_self(), kp->kp_proc.p_pid, &subject);
	if (status != KERN_SUCCESS) {
		sprintf(log_buffer, "task_for_pid: %s",
			mach_error_string(status));
		return PBSE_SYSTEM;
	}

	info_count = TASK_BASIC_INFO_COUNT;
	status = task_info(subject, TASK_BASIC_INFO,
		(task_info_t)&TaskInfo, &info_count);
	if (status != KERN_SUCCESS) {
		sprintf(log_buffer, "task_info: %s",
			mach_error_string(status));
		return PBSE_SYSTEM;
	}

	kp->kp_eproc.e_vm.vm_rssize = TaskInfo.resident_size;
	kp->kp_eproc.e_vm.vm_dsize = TaskInfo.virtual_size;
	kp->kp_eproc.e_vm.vm_tsize = 0;
	kp->kp_eproc.e_vm.vm_ssize = 0;

	ps->p_ru.ru_utime.tv_sec = TaskInfo.user_time.seconds;
	ps->p_ru.ru_utime.tv_usec = TaskInfo.user_time.microseconds;
	ps->p_ru.ru_stime.tv_sec = TaskInfo.system_time.seconds;
	ps->p_ru.ru_stime.tv_usec = TaskInfo.system_time.microseconds;

	status = task_threads(subject, &thread_list, &thread_count);
	if (status != KERN_SUCCESS) {
		sprintf(log_buffer, "task_threads: %s",
			mach_error_string(status));
		return PBSE_SYSTEM;
	}
	...
	for (i=0; i<thread_count; i++) {
		...
		if (status != KERN_SUCCESS) {
			sprintf(log_buffer, "thread_info: %s",
				mach_error_string(status));
			continue;
		}

		ps->p_ru.ru_utime.tv_sec += ThreadInfo.user_time.seconds;
		ps->p_ru.ru_utime.tv_usec += ThreadInfo.user_time.microseconds;
		ps->p_ru.ru_stime.tv_sec += ThreadInfo.system_time.seconds;
		ps->p_ru.ru_stime.tv_usec +=
			ThreadInfo.system_time.microseconds;
	}
	status = vm_deallocate(mach_task_self(),
		(vm_address_t)thread_list, sizeof(thread_list)*thread_count);
	if (status != KERN_SUCCESS) {
		sprintf(log_buffer, "vm_deallocate: %s",
			mach_error_string(status));
		return PBSE_SYSTEM;
	}
	return PBSE_NONE;
...
pcputime(char *id, int i)
{
	...
	if (pp->kp_proc.p_stat != SZOMB) {	/* got a live one */
		struct	pstats	*ps = &pstat_tbl[i];
	...
	else if (pp->kp_proc.p_ru != NULL) {	/* undead */
		struct	rusage	ru;		/* exit info */

		if (kvm_read(kd, (u_long)pp->kp_proc.p_ru, &ru,
			sizeof(ru)) != sizeof(ru))
			log_err(-1, id, kvm_geterr(kd));
		else
...
mom_get_sample()
{
	...
	if (proc_tbl == NULL) {
		sprintf(log_buffer, "kvm_getprocs: %s", kvm_geterr(kd));
	...
	pstat_tbl = (struct pstats *)calloc(nproc, sizeof(struct pstats));
	if (pstat_tbl == NULL) {
		sprintf(log_buffer, "can't allocate memory for pstat table");
		log_err(errno, __func__, log_buffer);
		return (PBSE_SYSTEM);
	}
	...
	for (i=0, kp=proc_tbl; i<nproc; i++, kp++) {
		if (kp->kp_eproc.e_sess != NULL) {
			if (kvm_read(kd, (u_long)kp->kp_eproc.e_sess,
				&ss, sizeof(ss)) != sizeof(ss)) {
				sprintf(log_buffer, "session: %s",
					kvm_geterr(kd));
...
#ifdef	__APPLE__
		{
			...
			/*
			 **	Have to read the real proc struct since
			 **	p_stats was not included in extern_proc.
			 */
			if (kp->kp_eproc.e_paddr == NULL)
				continue;

			if (kvm_read(kd, (u_long)kp->kp_eproc.e_paddr, &proc,
				sizeof(proc)) != sizeof(proc)) {
				sprintf(log_buffer, "proc: %s", kvm_geterr(kd));
				log_err(-1, __func__, log_buffer);
				continue;
			}
			pstats = (u_long)proc.p_stats;

			if (pstats != 0) {
				if (kvm_read(kd, pstats, &pstat_tbl[i],
					sizeof(struct pstats))
					!= sizeof(struct pstats)) {
					sprintf(log_buffer, "pstats: %s",
						kvm_geterr(kd));
					log_err(-1, __func__, log_buffer);
				}
			}

			if (apple_info(i) != PBSE_NONE) {
				log_err(-1, __func__, "apple_mach");
				continue;
			}
		}
#else
		pstat_tbl[i] = kp->kp_eproc.e_stats;
#endif
	}
...
/**
 * @brief
 *      wrapper function for resi_proc and resi_job..
 *
 * @param[in] attrib - pointer to rm_attribute structure
 *
 * @return      string
...
walltime(struct rm_attribute *attrib)
{
	...
	for (i=0; i<nproc; i++) {
		...
		found = 1;
		start = MIN(start, pstat_tbl[i].p_start.tv_sec);
	}
	if (found) {
...
quota(struct  rm_attribute *attrib)
{
	...
	/* all sizes in KB */
	switch (type) {
#ifdef	__APPLE__		/* values are in bytes not blocks */
		case harddata:
			sprintf(ret_string, "%lukb",
		...
		case harddata:
			sprintf(ret_string, "%lukb",
				(unsigned long)((qi.dqb_bhardlimit*DEV_BSIZE) >> 10));
			break;
		case softdata:
			sprintf(ret_string, "%lukb",
				(unsigned long)((qi.dqb_bsoftlimit*DEV_BSIZE) >> 10));
			break;
		case currdata:
			sprintf(ret_string, "%lukb",
				(unsigned long)((qi.dqb_curblocks*DEV_BSIZE) >> 10));
			break;
#endif
		case hardfile:
