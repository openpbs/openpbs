		...
		case IM_JOIN_JOB:
			...
			if (pjob->ji_nodeid == TM_ERROR_NODE) {
				...
				snprintf(log_buffer, sizeof(log_buffer),
					 ...
					 "possible network misconfiguration",
					 mom_host);
	...
	switch (command) {
		...
		case IM_DELETE_JOB_REPLY:
			...
			if (mom_process_hooks(
				    (command == IM_DELETE_JOB2) ? HOOK_EVENT_EXECJOB_EPILOGUE : HOOK_EVENT_EXECJOB_END,
		...
		case IM_POLL_JOB:
			...
			exitval = (pjob->ji_qs.ji_svrflags &
				   (JOB_SVFLG_OVERLMT1 | JOB_SVFLG_OVERLMT2))
					  ? 1
					  : 0;
		...
		case IM_RESUME:
			...
			if (check_ms(stream, pjob))
				goto fini;
			DBPRT(("%s: %s %s\n", __func__, (command == IM_SUSPEND) ? "SUSPEND" : "RESUME", jobid))
			sprintf(log_buffer, "%s received", (command == IM_SUSPEND) ? "SUSPEND" : "RESUME");
			log_event(PBSEVENT_JOB, PBS_EVENTCLASS_JOB, LOG_DEBUG,
				  jobid, log_buffer);
			if ((errcode = local_supres(pjob,
						    (command == IM_SUSPEND) ? 1 : 0, NULL)) != PBSE_NONE) {
				SEND_ERR(errcode);
				break;
			}
			pjob->ji_mompost = (command == IM_SUSPEND) ? post_suspend : post_resume;
			/*
		...
		case IM_CHECKPOINT_ABORT:
			...
			if (check_ms(stream, pjob))
				goto fini;
			DBPRT(("%s: %s %s\n", __func__,
			       (command == IM_CHECKPOINT) ? "CHECKPOINT" : "CHECKPOINT_ABORT", jobid))
			sprintf(log_buffer, "%s received",
				(command == IM_CHECKPOINT) ? "CHECKPOINT" : "CHECKPOINT_ABORT");
			log_event(PBSEVENT_JOB, PBS_EVENTCLASS_JOB, LOG_DEBUG,
				  jobid, log_buffer);
		...
		case IM_ALL_OKAY: /* this is a REPLY */
			...
			switch (event_com) {
				...
				case IM_RESUME:
					 ...
					 ** )
					 */
					name = (event_com == IM_SUSPEND) ? "SUSPEND" : "RESUME";
					if ((pjob->ji_qs.ji_svrflags & JOB_SVFLG_HERE) == 0) {
						sprintf(log_buffer,
				...
				case IM_CHECKPOINT_ABORT:
					 ...
					 ** )
					 */
					name = (event_com == IM_RESTART) ? "RESTART" : (event_com == IM_CHECKPOINT) ? "CHECKPOINT"
														    : "CHECKPOINT_ABORT";
					if ((pjob->ji_qs.ji_svrflags & JOB_SVFLG_HERE) == 0) {
						sprintf(log_buffer,
							"got %s OKAY and I'm not MS", name);
				...
				case IM_RESUME:
					...
					if ((pjob->ji_qs.ji_svrflags & JOB_SVFLG_HERE) == 0) {
						sprintf(log_buffer,
							...
							(event_com == IM_SUSPEND) ? "SUSPEND" : "RESUME");
						goto err;
					}
					sprintf(log_buffer, "%s returned ERROR %d",
						(event_com == IM_SUSPEND) ? "SUSPEND" : "RESUME", errcode);
