	...
	switch (event_type) {
		...
		case HOOK_EVENT_EXECJOB_PRETERM:
			...
			if (vnl == NULL) {
				...
				/* This looks into pjob->ji_vnods[] whose     */
				...
				if ((vnl_add_vnode_entries(vnl, pjob->ji_vnods,
					pjob->ji_numvnod,
					&matched_nvnode) == -1))
					goto run_hook_exit;

				/* This looks into pjob->ji_vnods0[] whose    */
				/* entries map those chunks in exec_vnode     */
				/* with ncpus=0.			      */
				/* For example, given			      */
				/*   exec_vnode=(hostA[0]:ncpus=0:mem=100kb)+ */
				/*              (hostB[0]:mem=400kb:ncpus=1)  */
				/*		(hostA[0]:ncpus=0:mem=200kb)  */
				/* vnl would end up getting accumulated       */
				/* entries:      			      */
				/*	(hostA[0],ncpus=0,mem=300kb)	      */
				/*	(hostB[0],ncpus=1,mem=400kb) 	      */

				if ((vnl_add_vnode_entries(vnl, pjob->ji_vnods0,
					pjob->ji_numvnod0,
					&matched_nvnode) == -1))
					goto run_hook_exit;
		...
		if (strcmp(obj_name, EVENT_OBJECT) == 0) {
			...
			if (start_new_vnl) {
				...
				if ((pvna != NULL) && (pvnalist != NULL)) {
					...
					log_err(errno, __func__,
						"Failed to allocate hook action");
				}
				if ((copy_file == 0) && (hvnlp != NULL)) {
					/* add a entry to the vnl for the user */
					rc=vn_addvnr(hvnlp, mom_short_name,
						VNATTR_HOOK_REQUESTOR,
						hook_euser, ATR_TYPE_STR, READ_ONLY, NULL);
					if (rc == -1) {
						snprintf(log_buffer, sizeof(log_buffer),
			...
			if (hvnlp) {
				...
				rc=vn_addvnr(hvnlp, vname_str, name_str,
	...
	size_t msg_len, int update_server)
{
	...
	for (phook = (hook *)GET_NEXT(*head_ptr); phook; phook = phook_next) {
		...
		if( ((hook_event == HOOK_EVENT_EXECJOB_END) || \
			...
			/*
			 * If there are multiple execjob_launch hooks, we need to cascade
			 * the execjob_launch specific parameters to the next execjob_launch
			 * hook, if any of the previous hooks has set these values.
			 */
			if (hook_output != NULL) {
