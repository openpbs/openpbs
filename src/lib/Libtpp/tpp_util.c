...
/* AVL tree of hostname cache - so that we can search faster inside it */
AVL_IX_DESC *AVL_addrcache = NULL;

pthread_mutex_t addrcache_lock;       /* global lock for the cache tree */
...
/**
 * @brief Initialize the tpp address cache
 *
 * @return      - Failure code
 * @retval   0  - Success
 * @retval  -1  - Failure
...
/**
 * @brief Look up an hostname in the cache, return associated address
 *        list if host found in cache
 *
 * @param[in] host - name of host to lookup
 * @param[out] count - count of addresses that was returned
 *
 * @return         - address list
 ...
 **/
tpp_addr_t *
tpp_lookup_addr_cache(char *host, int *count)
{
	...
	*count = 0;
	...
	if (rec) {
		if ((now - rec->update_time) > TPP_CACHE_EXPIRY_SECS) {
			...
			/* now unlock and return no hit */
			tpp_unlock(&addrcache_lock);
			return NULL;
		}

		*count = rec->count;
		addr_len = sizeof(tpp_addr_t) * rec->count;

		/* make a copy of the addrs array and return copy */
		addrs = malloc(addr_len);
		if (!addrs) {
		...
		memcpy(addrs, rec->addrs, addr_len);
	...
	/* no match, unlock and return */
	tpp_unlock(&addrcache_lock);
	return NULL;
}

/**
 * @brief Add a host and associate it to a list of addresses in the cache
 *
 * @param[in] host - name of host to lookup
 * @param[in] addrs - list of addresses to associate with host
 * @param[in] count - count of addresses passed in addrs
 *
 * @return     - Failure code
 * @retval   0 - success
 * @retval  -1 - Failure
 ...
 **/
int
tpp_set_addr_cache(char *host, tpp_addr_t *addrs, int count)
{
	struct cache_rec *rec;
	int rc;
	int addr_len;

	rec = malloc(sizeof(struct cache_rec));
	if (!rec) {
		tpp_log_func(LOG_CRIT, __func__, "Out of memory allocating address block");
		return -1;
	}
	...
	/* make a copy of the addrs array and return copy */
	rec->addrs = malloc(addr_len);
	if (!rec->addrs) {
		free(rec);
		tpp_log_func(LOG_CRIT, __func__, "Out of memory allocating address block");
		return -1;
	}
	memcpy(rec->addrs, addrs, addr_len);
...
tpp_netaddr(tpp_addr_t *ap)
{
	...
	if (tpp_log_event_mask >= (PBSEVENT_DEBUG4 - 1)) {
		...
		if (tpp_sock_resolve_ip(ap, host, sizeof(host)) == 0) {
			snprintf(tmp_buf, sizeof(tmp_buf), "(%s)%s", host, ptr->tppstaticbuf);
