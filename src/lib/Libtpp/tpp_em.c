...
tpp_em_init(int max_events)
{
	pollset_context_t *ctx = malloc(sizeof(pollset_context_t));
	if (!ctx)
		return NULL;
...
tpp_em_add_fd(void *em_ctx, int fd, int event_mask)
{
	...
	pctl.fd = fd;
	pctl.events = event_mask;
	pctl.cmd = PS_ADD;

	if (pollset_ctl(ctx->ps, &pctl, 1) != 0)
		return -1;
...
{
	/*
	 * PS_MOD cannot be used, since it OR's bits
	 * in the event_mask with existing bits
	 * Our application expects to "reset" the event_mask
	 * to the one provided, and not OR'ed. So we
	 * delete the fd and add it again
	 */

	tpp_em_del_fd(em_ctx, fd);
	return (tpp_em_add_fd(em_ctx, fd, event_mask));
}
...
tpp_em_init(int max_events)
{
	devpoll_context_t *ctx;
	int i;

	ctx = malloc(sizeof(devpoll_context_t));
	if (!ctx)
		return NULL;
	...
	if ((ctx->devpoll_fd = open("/dev/poll", O_RDWR)) == -1) {
		free(ctx->events);
		free(ctx);
		return NULL;
	}
	tpp_set_close_on_exec(ctx->devpoll_fd);
	ctx->max_nfds = max_events;
...
tpp_em_destroy(void *em_ctx)
{
	close(((devpoll_context_t *) em_ctx)->devpoll_fd);
	free(((devpoll_context_t *) em_ctx)->events);
	free(em_ctx);
}
...
tpp_em_add_fd(void *em_ctx, int fd, int event_mask)
{
	...
	while (1) {
		s = write(ctx->devpoll_fd, &pfd, sizeof(pfd));
		if (s == sizeof(pfd))
			break;
		if (s == -1 && !(errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR))
			return -1;
	}
	return 0;
}
...
tpp_em_pwait(void *em_ctx, em_event_t **ev_array, int timeout, const sigset_t *sigmask)
{
	...
	dvp.dp_fds = ctx->events;
	dvp.dp_nfds = ctx->max_nfds;
