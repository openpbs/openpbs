...
pg_db_prepare_job_sqls(pbs_db_conn_t *conn)
{
	...
	snprintf(conn->conn_sql, MAX_SQL_LENGTH, "update pbs.job set "
		...
		"where ji_jobid = $1 "
		"returning to_char(ji_savetm, 'YYYY-MM-DD HH24:MI:SS.US')  as ji_savetm");
	if (pg_prepare_stmt(conn, STMT_UPDATE_JOB_ATTRSONLY, conn->conn_sql, 2) != 0)
		return -1;
...
pg_db_save_jobscr(pbs_db_conn_t *conn, pbs_db_obj_info_t *obj, int savetype)
{
	...
	if ((pg_db_cmd(conn, STMT_INSERT_JOBSCR, 2)) == -1)
		return -1;
 ...
 */
int
pg_db_del_attr_job(pbs_db_conn_t *conn, void *obj_id, char *sv_time, pbs_db_attr_list_t *attr_list)
{
	char *raw_array = NULL;
	int len = 0;
	static int ji_savetm_fnum;
	...
	if (pg_db_cmd(conn, STMT_REMOVE_JOBATTRS, 2) == -1) {
		free(raw_array);
		return -1;
	}
	...
	GET_PARAM_STR(conn->conn_resultset, 0, sv_time, ji_savetm_fnum);
	PQclear(conn->conn_resultset);
	free(raw_array);
