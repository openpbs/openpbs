 ...
 * @file	enc_Delete.c
 * @brief
 * encode_DIS_Delete() - encode a Delete Batch Request
 *
...
int
encode_DIS_Delete(int sock, int command, int objtype, char **objname, struct attropl *aoplp)
{
	int   rc;

	if ((rc = diswui(sock, command) != 0) ||
		(rc = diswui(sock, objtype) != 0) ||
		(rc = encode_DIS_JobsList(sock, objname) != 0))
			return rc;
