...
bld_wenv_variables(char *name, char *value)
{
	...
	if (env_array == NULL) {
		...
		if (env_array == NULL) {
			log_err(errno, __func__, "Failed to allocate memory for env_array");
			return;
		}
...
mktmpdir(char *jobid, char *username)
{
	...
	if (_mkdir(tmpdir) == -1) {
		...
		if (errno == ERROR_ALREADY_EXISTS)
			return 0;
		sprintf(log_buffer, "%s: mkdir: %s : error: %d",
			jobid, tmpdir, errno);
		log_err(errno, __func__, log_buffer);
	...
	*/
	...
	if ((is_interactive_job) || ((pjob->ji_numnodes > 1) && (!nodemux))) {	/* mom_open_demux mechanism available */
		...
		if (SetFilePointer(si.hStdOutput, (LONG)NULL,
			...
			log_err(-1, __func__, "Unable to SetFilePointer");
		if (SetFilePointer(si.hStdError, (LONG)NULL,
			(PLONG)NULL, FILE_END) == INVALID_SET_FILE_POINTER)
			log_err(-1, __func__, "Unable to SetFilePointer");
	}
