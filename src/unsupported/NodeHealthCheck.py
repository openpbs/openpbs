...
#
# Altair’s dual-license business model allows companies, individuals, and
# rganizations to create proprietary derivative works of PBS Pro and
# distribute them - whether embedded or bundled with other software -
# under a commercial license agreement.
#
# Use of Altair’s trademarks, including but not limited to "PBS™",
...
#    py_path = '/opt/pbs/default/python/lib'
    ...
    my_paths = [ py_path+'/python27.zip', \
                     py_path+'/python2.7', \
                     py_path+'/python2.7/plat-linux2', \
                     py_path+'/python2.7/lib-tk', \
                     py_path+'/python2.7/lib-dynload', \
                     py_path+'/python2.7/site-packages']

    if sys.path.__contains__(py_path+'/python2.7') == False:
    ...
    def ChkDiskUsage(self):
        ...
        for check_dir in self.nhc_cfg["disk_space"]["dirs"]:
            ...
            try:
                ...
                return [self.nhc_cfg["disk_space"]["dirs"][check_dir][1],"No file or directory: %s"%check_dir]
            except Exception, e:
            ...
            if isinstance(spaceVal,(int,long)):
                pbs.logmsg(pbs.EVENT_DEBUG3,"Free: %0.2lfgb\tRequested: %0.2lfgb"%(float(free)/float(gb_unit),float(spaceVal)/float(gb_unit)))
                if free < spaceVal:
                    return [self.nhc_cfg["disk_space"]["dirs"][check_dir][1],'%s failed disk space check. Free: %0.2lfgb\tRequested: %0.2lfgb'%(check_dir,float(free)/float(gb_unit),float(spaceVal)/float(gb_unit))]

            elif isinstance(spaceVal,(float)):
                try:
                    pbs.logmsg(pbs.EVENT_DEBUG3,"Free: %d\tTotal: %d\tUsed: %d\tUsed+Free: %d\tSpaceVal: %d"%(free,total,used,used+free,int(spaceVal)))
                    percent = 100 - int((float(used) / float(used+free)) * 100)
